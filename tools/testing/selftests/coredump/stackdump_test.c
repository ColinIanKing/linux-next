...
static ssize_t recv_marker(int fd)
{
	...
	case COREDUMP_MARK_INVALIDSIZE:
		fprintf(stderr, "Received marker: InvalidSize\n");
		return COREDUMP_MARK_INVALIDSIZE;
...
TEST_F(coredump, socket_request_invalid_size_small)
{
	...
	if (pid_coredump_server == 0) {
		...
		if (!read_marker(fd_coredump, COREDUMP_MARK_INVALIDSIZE))
			goto out;
