...
TEST_F_FORK(layout1_bind, path_disconnected_rename)
{
	...
	int ruleset_fd_l1 = create_ruleset(_metadata, ACCESS_ALL, layer1);
	int ruleset_fd_l2 = create_ruleset(
		_metadata, LANDLOCK_ACCESS_FS_READ_FILE, layer2_only_s1d2);
	pid_t child_pid;
	int bind_s1d3_fd, status;

	EXPECT_LE(0, ruleset_fd_l1);
	EXPECT_LE(0, ruleset_fd_l2);
	...
	EXPECT_EQ(0, test_open_rel(bind_s1d3_fd, file2_name, O_RDONLY));
	EXPECT_EQ(0, renameat(bind_s1d3_fd, file2_name, AT_FDCWD, file1_s2d2))
	{
