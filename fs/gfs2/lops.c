...
void gfs2_log_submit_write(struct gfs2_sbd *sdp, struct bio **biop)
{
	struct bio *bio = *biop;
	if (bio) {
				    ...
				    bool flush)
{
	...
	if (bio) {
		...
		if (blkno == nblk && !flush)
			return bio;
		gfs2_log_submit_write(sdp, biop);
	}

	*biop = gfs2_log_alloc_bio(sdp, blkno, opf);
	return *biop;
}
		    ...
		    u64 blkno, blk_opf_t opf)
{
	...
	bio = gfs2_log_get_bio(sdp, blkno, &jd->jd_log_bio, opf, false);
	ret = bio_add_page(bio, page, size, offset);
	if (ret == 0) {
		bio = gfs2_log_get_bio(sdp, blkno, &jd->jd_log_bio, opf, true);
		ret = bio_add_page(bio, page, size, offset);
		WARN_ON(ret == 0);
	}
}
...
static void gfs2_log_submit_read(struct gfs2_sbd *sdp, struct bio *bio)
{
	bio->bi_private = sdp;
	bio->bi_end_io = gfs2_end_log_read;
	submit_bio(bio);
}
