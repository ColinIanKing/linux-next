			       ...
			       const unsigned int xid)
{
	...
	sillycounter++;
	scnprintf(to_name, len, "%s%c%s%04X", ppath,
		  CIFS_DIR_SEP(cifs_sb), ".__smb", sillycounter);
	...
	iov[1].iov_len = sizeof(*utf16_path) * UniStrnlen(utf16_path, PATH_MAX);
	iov[2].iov_base = &(__u8){1}; /* DeletePending */
	iov[2].iov_len = sizeof(__u8);
