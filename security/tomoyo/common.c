...
/**
 * tomoyo_digits_len - Calculate length of leading decimal digits in a string.
 *
 * @str: String to scan.
 *
 * Returns length of leading decimal digits in @str.
 */
...
static void tomoyo_patternize_path(char *buffer, const int len, char *entry)
{
	...
	*cp++ = '\0';
	...
	if (!strncmp(cp, "pipe:[", 6)) {
		width = tomoyo_digits_len(cp + 6);
		if (width > 0 && *(cp + width + 6) == ']') {
			cp += width + 7;
	...
	if (!strncmp(cp, "socket:[", 8)) {
		width = tomoyo_digits_len(cp + 8);
		if (width > 0 && *(cp + width + 8) == ']') {
			cp += width + 9;
	...
	if (!strncmp(cp, "/fd/", 4)) {
		width = tomoyo_digits_len(cp + 4);
		if (width >= 1) {
			cp += width + 4;
			tomoyo_addprintf(buffer, len, "/fd/\\$");
		}
	} else if (!strncmp(cp, "/fdinfo/", 8)) {
		width = tomoyo_digits_len(cp + 8);
		if (width >= 1) {
			cp += width + 8;
...
flush:
	/* Flush remaining part if any (e.g, $CMD part of "file ioctl" entry). */
	if (*cp)
